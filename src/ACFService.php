            ...
            foreach ($values as $ev) {
                ...
                if (empty($relation)) {
                    if (empty($post_types)) {
	                    ...
	                    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
                        $relation = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE post_type IN ($format) AND ( post_title = %s OR post_name = %s )", $post_types, $ev, sanitize_title_for_query($ev)));
                    }
                }
                if ($relation) {
